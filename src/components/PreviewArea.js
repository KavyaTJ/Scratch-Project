...
export default function PreviewArea({ sprites }) {
  ...
  const executeAnimation = async (sprite, animations) => {
    ...
    for (const animation of animations) {
      ...
      // Add a short delay between each animation step
      await new Promise(resolve => setTimeout(resolve, 500));
    }
  
    // Check for collisions after completing all animations
    sprites.forEach(otherSprite => {
      if (sprite.id !== otherSprite.id && checkCollision(sprite, otherSprite)) {
        [sprite.animations, otherSprite.animations] = [otherSprite.animations, sprite.animations];
      }
    });
  };
